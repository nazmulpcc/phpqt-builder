...
if (!qt_runtime_php_signal_env_enabled('PHPQT_ENABLE_HEAVY_STRESS')) {
    qt_runtime_skip('Heavy PHP signal stress is opt-in. Set PHPQT_ENABLE_HEAVY_STRESS=1 to run it.');
}
...
$threadCount = qt_runtime_php_signal_env_int('PHPQT_HEAVY_STRESS_THREADS', 8);
$messageCount = qt_runtime_php_signal_env_int('PHPQT_HEAVY_STRESS_MESSAGES', 48);
$roundCount = qt_runtime_php_signal_env_int('PHPQT_HEAVY_STRESS_ROUNDS', 4);
$messagesPerReceiver = 1 + ($messageCount * $roundCount);
$expectedTotalHits = $threadCount * $messagesPerReceiver;
$timeoutMs = qt_runtime_php_signal_env_int(
    'PHPQT_HEAVY_STRESS_TIMEOUT_MS',
    max(20000, min(300000, 15000 + ((int) ceil($expectedTotalHits / 75000)) * 1000))
);

$messages = ['heavy-0'];
for ($round = 1; $round <= $roundCount; $round++) {
    for ($i = 1; $i <= $messageCount; $i++) {
        $messages[] = 'heavy-' . $round . '-' . $i;
    }
}
